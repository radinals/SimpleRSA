...
mpz_class
SimpleRSA::randomRangeNumberGenerator(mpz_class min, mpz_class max)
{
	mpz_class rand_num;
...
bool
SimpleRSA::isFermatPrime(mpz_class number, mpz_class k)
{
	if (number <= 1) {
		return false;
	}
...
void
SimpleRSA::generate_key(mpz_class p, mpz_class q)
{
	m_p = p;
	m_q = q;
...
mpz_class
SimpleRSA::encryptChar(mpz_class plain_char)
{
	mpz_class result;

	// result = ((character) ** (m_public_key)) mod (m_n)
	mpz_powm(result.get_mpz_t(), plain_char.get_mpz_t(),
	         m_public_key.get_mpz_t(), m_n.get_mpz_t());

	return result;
}

mpz_class
SimpleRSA::decryptChar(mpz_class cypher_char)
{
	...
	mpz_powm(result.get_mpz_t(), cypher_char.get_mpz_t(),
	         m_private_key.get_mpz_t(), m_n.get_mpz_t());

	return result;
}

RSAText
SimpleRSA::decrypt(RSAText text)
{
...
RSAText
SimpleRSA::encrypt(RSAText text)
{
