...
std::string RSAText::operator=(const std::string &string)
{
    if (!m_charlist.empty())
        m_charlist.clear();
...
void RSAText::foreachChar(std::function<void(mpz_class &ch)> action)
{
    std::for_each(m_charlist.begin(), m_charlist.end(), action);
}

std::string RSAText::getString()
{
    std::string tmp;
...
std::string RSAText::getAscii(std::string separator)
{
    std::string tmp;
    for (mpz_class ch : m_charlist) {
